...
> **Release status:** `1.2.0` is the stable release. It adds a visible approval gate for material
> support machinery and uses simpler international English in user-facing responses. Core
> workflows, direct plan-derived progress, project tracking, scope control, legacy-state migration,
> and source-documentation checks remain present. Migration has been exercised on JSVision with its
> full cross-platform CI matrix. The retained Claude 3.12.0 benchmark remains requirements-stage
> evidence and is not a claim of complete comparative parity.
