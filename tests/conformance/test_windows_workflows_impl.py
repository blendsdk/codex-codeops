...
class DocumentationImplementationTests(unittest.TestCase):
    def test_windows_docs_publish_supported_boundary_and_native_commands(self) -> None:
        combined = "\n".join(
            (ROOT / relative).read_text(encoding="utf-8")
            for relative in ("README.md", "docs/installation.md", "docs/troubleshooting.md")
        )
        self.assertIn("release-tested hosts", combined)
        self.assertIn("native Windows 11", combined)
