...
The closed ST-1–ST-54 inventory is the coverage authority. Each named component behavior and error
class maps to at least one ST case, and the canonical module map below assigns every ST exactly one
primary specification owner. Test collection and the full verification dispatcher fail when a
declared ST case or primary module is missing. No percentage or separate undeclared fault-ID
inventory is used as completion evidence.
...
| ST-2 | `py -3` is absent, but `python` runs Python 3.10 or newer | Bootstrap selects `python`, exits 0, and records the observed interpreter without a patch pin | RD-022; AR-3; 03-01 §Proposed Changes |
| ST-3 | Both interpreter commands are absent, broken, or report Python older than 3.10 | Exit 2 with BLOCKED `python-3`, actionable install/configuration guidance, no auto-install attempt, no attestation, and no project mutation | RD-022; AR-3; 03-01 §Error Handling |
...
| ST-5 | A mutating entrypoint's passive host adapter reports that the current process is running under WSL | Refuse before host dispatch with guidance to launch native Codex; perform no mutation and never invoke WSL to confirm the fixture | RD-022; AR-4; 03-01 §Error Handling |
...
| ST-8 | Workspace is removable, non-NTFS, UNC/network, outside containment, not writable, or has any existing reparse component from root through a mutation target | Exit 2 with the corresponding BLOCKED local-workspace/path check before mutation | RD-022; AR-11; 03-01 §Required Checks |
...
| ST-11 | Input contains hostile session/path text, an unknown mode/entrypoint, missing/duplicate/out-of-root mutation targets, session/read targets, a type error, or an extra field | Reject malformed input with exit 1; perform no path escape, command execution, attestation write, or project mutation | AR-6, AR-11; 03-01 §Mode and Input Contract |
| ST-12 | A direct CLI mutation, trusted-hook mutation, and mutating skill each start with and without hook trust, including a passively classified WSL fixture | The registered command boundary rejects WSL before host dispatch and reruns all ten checks before its first native-Windows write; hooks/skills are defense in depth; an unknown/missing command entrypoint is rejected | RD-022; AR-4, AR-5; 03-01 §Hook Integration |

### Process Ownership, Paths, and Atomic State

| # | Input / Scenario | Expected Output / Behavior | Source |
|---|---|---|---|
| ST-13 | A transition begins on native Windows with a queryable current process | Journal/lock owner uses `{schemaVersion, backend: windows-filetime, pid, creationFileTime}` with the exact decimal `FILETIME`; transition can proceed | RD-022; AR-9; 03-03 §Process Identity Contract |
...
| ST-23 | Input contains traversal, drive/UNC/device/ADS syntax, invalid characters, reserved device basename, trailing dot/space, a root or nested target-parent reparse component, or an NTFS alias collision | Reject before mutation; existing state is not silently rewritten | RD-022; AR-11; 03-03 §Path Contract |
| ST-24 | Atomic replace receives WinError 32/33 and then succeeds on the fixed schedule | Retry only according to `(0.05, 0.10, 0.20, 0.40, 0.75)`, commit valid bytes, and preserve protocol ordering | AR-10; 03-03 §Atomic Write Contract |
...
| ST-26 | Replacement fails with generic `ACCESS_DENIED`, validation, path safety, permission policy, or an unknown OS error | Do not retry; fail closed with the original destination/recovery evidence protected | AR-10, AR-11; 03-03 §Atomic Write Contract |
| ST-27 | Preflight runs on removable/non-NTFS/reparse-backed/UNC/network storage | Return BLOCKED before a workflow mutation starts | RD-022; 03-01 §Required Checks; 03-03 §Atomic Write Contract |
...
| ST-39 | The shared subprocess evidence sink captures every spawned command during representative Windows workflows | No normalized executable/argument invokes `bash`, Git Bash, `wsl`, a distribution, or `/mnt` translation; ordinary runs retain no trace | RD-022; AR-4; 03-04 §CI Contract |
...
| ST-43 | Unicode, long-but-supported, case-aliased, and 8.3-aliased contained paths run through state, migration, roadmap, and worktree commands | Safe paths round-trip canonically; aliases that resolve to one NTFS object collide and block; no truncation occurs | RD-022; 03-03 §Path Contract |
...
| ST-45 | Any readiness condition changes before the command's first write, or filesystem check 10 changes before atomic replace/recovery | The command reruns all ten checks; the writer repeats check 10; failure refuses mutation and preserves prior state | RD-022; AR-5; 03-01 §Proposed Changes; 03-03 §Integration Points |
| ST-46 | CI configuration is validated | Ubuntu and explicit `windows-11-arm` jobs each use one Python 3.10+; Windows commands are PowerShell/Python and both run all five logical gates | AR-12, AR-14; 03-04 §CI Contract |
...
| ST-50 | The capture command produces version-matched Windows 11 CLI evidence, command trace, sanitized scenario records, and matching hashes for every required scenario | CLI evidence gate recomputes bindings and passes | RD-023; AR-13; 03-04 §Evidence Manifest |
| ST-51 | Version-matched Windows desktop installation/hook/representative-workflow smoke includes reviewer identity, UTC timestamp, candidate hash, Codex version, and checklist results | Desktop evidence gate passes | RD-023; AR-13; 03-04 §Evidence Manifest |
...
| ST-53 | Documentation permits Python older than 3.10, pins an exact patch, or says WSL must be uninstalled | Documentation validation fails | RD-022; AR-3, AR-4; 03-04 §Documentation and Release Gate |
...
|---|---|---|
| `tests/conformance/test_windows_preflight_spec.py` | ST-1–ST-11, ST-28–ST-30 | Preflight and hooks |
| `tests/conformance/test_windows_state_spec.py` | ST-13–ST-27, ST-37, ST-41–ST-45 | State and filesystem |
| `tests/conformance/test_portable_utilities_spec.py` | ST-31–ST-38 | Portable control layer |
| `tests/conformance/test_windows_workflows_spec.py` | ST-12, ST-39–ST-40; regression reuse of ST-35, ST-36 | Workflow integration |
...
| `tests/conformance/test_windows_workflows_impl.py` | Command-boundary gates, interpreter policy, command capture, installed paths | High |
...
- [ ] Every declared ST case is collected by its canonical primary module.
